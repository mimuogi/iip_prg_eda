...
public class EditorPredictivo extends ABB<String> {
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {
        String prefijoPrimordial = prefijo;
        int encontrados = (recuperar(prefijo) != null) ? 1 : 0;
        ListaConPI<String> res = new LEGListaConPI<String>();
        res.inicio();
        if(encontrados == 1) res.insertar(prefijo);
        while(encontrados < n) 
        {
            prefijo = sucesor(prefijo);
            if(prefijo != null && prefijo.startsWith(prefijoPrimordial))
            {
                encontrados++;
