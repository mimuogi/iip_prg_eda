...
public class GrafoRedMunicipiosEx {
    ...
    // TURNO 3
    public Municipio adyacenteMenorPeso(Municipio m) {
        Arista [] kruskal = gRM.kruskal();
        Integer v = getVertice(m);
        ListaConPI<Adyacente> l = gRM.adyacentesDe(v);
        l.inicio();
        while(!l.esFin()){
            Adyacente aMin = l.recuperar();
            Municipio res = getMunicipio(aMin.getDestino());
            for(int i = 0; i < kruskal.length; i++){
                if(v != null && 
                    kruskal[i].getOrigen() == v 
                    && kruskal[i].getDestino() == aMin.getDestino()) {
                    return res;
                }
            }
            l.siguiente();
