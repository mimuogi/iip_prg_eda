...
public class IndexadorEx {
    ...
    public ListaConPI<String> palsFrecuenciaUno(ListaConPI<String> l) {
        ...
        Map<String, Integer> map = new TablaHash<String, Integer>(l.talla());
        String pal = null; String pal1 = null;
        l.inicio();
        while(!l.esFin()){
            pal = l.recuperar();
            Integer x = map.recuperar(pal);
            if (x == null) {map.insertar(pal,1);}
            else {map.insertar(pal,x++);}
            l.siguiente();
        }
        ...
        while(!claves.esFin()){
            pal1 = claves.recuperar();
            Integer x = map.recuperar(pal1);
            if (x == freq) {
                res.insertar(pal1);
            }
            claves.siguiente();
